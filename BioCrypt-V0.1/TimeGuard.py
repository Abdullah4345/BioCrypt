...
class ModernGUI(tk.Tk):
    ...
    def _create_file_management_tab(self):
        ...
        ttk.Label(settings_frame, text="Expiration Date:").grid(row=0, column=0)
        self.calendar = Calendar(settings_frame, selectmode='day', date_pattern='y-mm-dd')
        self.calendar.grid(row=0, column=1, padx=10)
        
        ttk.Button(settings_frame, text="Set Time", command=self._show_time_picker).grid(row=0, column=2, padx=10)
        
        self.time_frame = ttk.Frame(settings_frame)
        self.time_frame.grid(row=1, column=0, columnspan=3, pady=10)
        self.time_frame.grid_remove()  # Initially hide the time frame
        
        ttk.Label(self.time_frame, text="Hour:").grid(row=0, column=0)
        self.hour_spinbox = ttk.Spinbox(self.time_frame, from_=0, to=23, font=self.text_font, width=5)
        self.hour_spinbox.grid(row=0, column=1, padx=10)
        
        ttk.Label(self.time_frame, text="Minute:").grid(row=0, column=2)
        self.minute_spinbox = ttk.Spinbox(self.time_frame, from_=0, to=59, font=self.text_font, width=5)
        self.minute_spinbox.grid(row=0, column=3, padx=10)
        
        ttk.Label(self.time_frame, text="Second:").grid(row=0, column=4)
        self.second_spinbox = ttk.Spinbox(self.time_frame, from_=0, to=59, font=self.text_font, width=5)
        self.second_spinbox.grid(row=0, column=5, padx=10)
        
        ttk.Label(settings_frame, text="Max Access Attempts:").grid(row=2, column=0)
        self.max_attempts = ttk.Spinbox(settings_frame, from_=1, to=10, font=self.text_font)
        self.max_attempts.grid(row=2, column=1, padx=10)
    ...
    def _send_otp_email(self, email, otp):
        try:
            sender_email = "omarfergany84@gmail.com"
            sender_password = "iaxc trzg yggz vwlk"
