...
class ChatBotApp(ctk.CTk):
    def __init__(self):
        ...
        # User input field
        self.user_input = ctk.CTkEntry(self, corner_radius=15, placeholder_text="Type your message here...")
        self.user_input.grid(row=1, column=0, padx=10, pady=10, sticky="ew")
        self.user_input.bind("<Return>", lambda event: self.process_input())

        # Send button
        self.send_button = ctk.CTkButton(self, text="Send", command=self.process_input, corner_radius=15)
        self.send_button.grid(row=1, column=1, padx=10, pady=10)

        self.display_message("ChatBot", "Hello! How can I assist you?", is_user=False)
    ...
    def process_input(self):
        ...
        self.display_message("ChatBot", response, is_user=False)
        self.user_input.delete(0, "end")

    def display_message(self, sender, message, is_user):
        user_bg_color = "#0078D7"  # Blue for user messages
        bot_bg_color = "#333333"   # Dark gray for bot messages
        ...
        msg_label = ctk.CTkLabel(
            ...
            text="",
            corner_radius=10,
            fg_color=user_bg_color if is_user else bot_bg_color,
            text_color="white",
            padx=10, pady=5,
            wraplength=280
        )
        ...
        self.animate_message(msg_label, message, 0)
        
    def animate_message(self, label, message, index):
        """Gradually displays the message one letter at a time."""
        if index < len(message):
            label.configure(text=label.cget("text") + message[index])
            self.after(10, lambda: self.animate_message(label, message, index + 1))
    ...
    def query_gemini(self, user_input):
        try:
            ...
            context = """
                ...
                Advanced Security with IPFS & Blockchain

                In BioCrypt, we leverage a highly advanced security mechanism using the InterPlanetary File System (IPFS). This technology fragments each file into thousands of smaller encrypted pieces and distributes them across a decentralized network. Instead of storing the entire file in one location, the IPFS network ensures that even if a hacker gains access to one fragment, they cannot reconstruct the file without the unique Content Identifier (CID).

                To further enhance security, we hide the CID inside a blockchain (Ethereum). The Ethereum blockchain is practically unhackable because any modifications or unauthorized access attempts must be validated by millions of computers worldwide. This makes BioCrypt one of the most secure file storage and encryption systems, leveraging both decentralization and cryptographic validation.

                This method prevents single-point failures, enhances privacy and anonymity, and ensures that only authorized users with the correct decryption key and CID can reconstruct and access the original file.

                The combination of biometric encryption, IPFS decentralized storage, and blockchain security makes BioCrypt a groundbreaking technology in data protection.
