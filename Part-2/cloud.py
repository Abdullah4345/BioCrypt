...
class GoogleDriveUploader:
    ...
    def load_mapping(self):
        ...
        try:
            with open('file_mapping.csv', mode='r') as file:
                reader = csv.reader(file)
    ...
    def authenticate(self):
        ...
        if not self.creds or not self.creds.valid:
            if self.creds and self.creds.expired and self.creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file('client_secret_800944173395-66ueils05h7sf7ois00jvlv0g3276o82.apps.googleusercontent.com.json', SCOPES)
    ...
    def upload_file(self, file_path, user_email):
        ...
        try:
            ...
            with open('file_mapping.csv', mode='a', newline='') as file:
...
class FileManagerApp:
    def __init__(self, root):
        ...
        # Title Label
        self.title_label = tk.Label(root, text="Google Cloud",bg=var1["bg"],fg=var2["fg"], font=("Courier", 55, "bold"))
        self.title_label.pack(pady=2)


        style = ttk.Style()
        style.configure("Treeview", background=var2["fg"], fieldbackground=var2["fg"], foreground=var1["bg"])

        # File List Treeview
        self.tree = ttk.Treeview(root, columns=("ID", "Name"), show="headings", height=15,)
        ...
        self.upload_button = tk.Button(self.buttons_frame, text="     Upload File      ", command=self.upload_file,bg=var1["bg"],fg=var2["fg"],relief="flat",bd=0,highlightthickness=0, font=("Courier", 12))
        self.download_button = tk.Button(self.buttons_frame, text="Download Selected File", command=self.download_file,bg=var1["bg"],fg=var2["fg"],relief="flat",bd=0,highlightthickness=0, font=("Courier", 12))
        self.delete_button = tk.Button(self.buttons_frame, text=" Delete Selected File " , command=self.delete_file,bg=var1["bg"],fg=var2["fg"],relief="flat",bd=0,highlightthickness=0, font=("Courier", 12))
        self.refresh_button = tk.Button(self.buttons_frame, text="       Refresh        ", command=self.refresh_file_list,bg=var1["bg"],fg=var2["fg"],relief="flat",bd=0,highlightthickness=0, font=("Courier", 12))
    ...
    def upload_file(self):
        ...
        if entered_otp == otp:
            ...
            if file_path:
                ...
                if file_id:
                    messagebox.showinfo("Success", f"File uploaded successfully! File ID: {file_id}")
                else:
    ...
    def ask_otp(self):
        ...
        otp = simpledialog.askstring("OTP", "Please enter the OTP sent to your email:")
        return otp
    ...
    def download_file(self):
        ...
        if not email:
            messagebox.showerror("Error", "No associated email found for this file.")
            return
        ...
        if entered_otp == otp:
            ...
            if save_path:
                success, message = self.uploader.download_file(file_id, save_path, email)
                if success:
